...
export default function BasicInfoTab({ appId }: { appId: string }) {
  ...
  const onFinish = (values: { name: string; description: string; tenantId: string }) => {
    updateApp(appId, { name: values.name, description: values.description, tenantId: values.tenantId })
